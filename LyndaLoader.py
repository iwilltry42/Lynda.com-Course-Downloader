...
def name_the_video(video_url, data_conviva):
    index = re.findall("SD/\S+.mp4", video_url)
...
def findToken(html): # find -_- for formdata
...
def findLyndaLoginStatus(txt):
    status = re.findall("LyndaLoginStatus=\S+;", txt)
    return status


def getCourseTitleAndCreateFolder(html, download_path, course_id):
    soup = BeautifulSoup(html, "lxml")  # "lxml" OS-specific, best for Windows
...
def main():
    ...
    courseID = str(re.findall("data-course-id=\"\S+\"", html)).split("\"")[1]
    print("Course-ID: ", courseID)

    download_path = getCourseTitleAndCreateFolder(result_new.text, download_path, courseID)
